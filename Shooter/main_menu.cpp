...
void main_menu::get_mouse_input()
{
	...
	if(isHoweringText2&&menu_event.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP)
		isDone=true;
	if(isHoweringText3&&menu_event.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP)
		level_editor();
	else if(increasing_in_progress1||increasing_in_progress1||decreasing_in_progress1||decreasing_in_progress2||increasing_in_progress3||decreasing_in_progress3||decreasing_in_progress_all)
		main_menu::set_mouse_position();
}
...
void main_menu::check_possible_interactions()
{
	if(mouse_x > 450 && mouse_x < 500 && mouse_y > 300 && mouse_y < 350)
			{
				...
				isHoweringText3=false;
				start_decreasing_font_size(2);
			}
	else if(mouse_x > 450 && mouse_x < 500 && mouse_y > 460 && mouse_y < 560)
			{
				draw_text(2);
	...
	else if(mouse_x > 450 && mouse_x <500 && mouse_y > 380 && mouse_y <460)
			{
				draw_text(3);
				isHoweringText3=true;
				isHoweringText1=false;
				isHoweringText2=false;
				//start_decreasing_font_size(3);
...
void main_menu::draw_text(int text_to_highlight)
{
	...
	if(change_font)
		...
		if(redraw_text)
		{
			...
			if(text_to_highlight==3)
				al_draw_text(menu_font_1[FontSize3],al_map_rgb(150,100,33),Xtext3,Ytext3,ALLEGRO_ALIGN_CENTRE,"Level Editor");
			else
				al_draw_text(menu_font_1[FontSize3],al_map_rgb(100,100,33),Xtext3,Ytext3,ALLEGRO_ALIGN_CENTRE,"Level Editor");
...
void main_menu::start_increasing_font_size(int font_number)
{
	...
	if(font_number==3)
	{
		increasing_in_progress3=true;
		if(FontSize3<size_to_reach)
		FontSize3++;
		if(FontSize3==size_to_reach)
		increasing_in_progress3=false;
	}
}
void main_menu::start_decreasing_font_size(int font_number)
{
	...
	if(font_number==3)
	{
		decreasing_in_progress3=true;
		if(FontSize3!=0)
		FontSize3--;
		if(FontSize3==0)
		decreasing_in_progress3=false;
	}
	if(!isHoweringText1&&!isHoweringText2&&!isHoweringText3)
	{
